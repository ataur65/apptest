...
import { HeroSlide } from '@/lib/interfaces';

interface HeroSectionClientProps {
  heroSlides: HeroSlide[];
}

const HeroSectionClient = ({ heroSlides }: HeroSectionClientProps) => {
    ...
    if (heroSlides && heroSlides.length > 1) {
      const interval = setInterval(() => {
        setCurrentSlide((prevSlide) =>
          prevSlide === (heroSlides?.length || 0) - 1 ? 0 : prevSlide + 1
        );
  ...
  if (!heroSlides || heroSlides.length === 0) return <p>No hero slides configured.</p>;
